...
int allocate = 2000;  //设定的一个可以分配内存的起点
...
namespace Parser {
    ...
    string  skip(string str) {
        ...
        for(char & c: str){
            if(c == ' ') continue;
            if(c == '/' || c == '\n') return ans;   //for line comments, we tos it away. so as '\n'
            ans += c;
        }
        return ans;
    }
...
bool handle(string & filename){
    ...
    while(getline(ifs, line)){
        if(line.size() == 1 && (line[0] >= 0 && line[0] <= 31)) continue;  //这一句意思是控制字符都去除，主要是一开始没分清换行和回车，控制字符分布区间为[0,31]
        ...
        if(commandtype == '/') continue;
            ...
            int i = 1;
            string symbol;
            while(i < line1.size() && line1[i] != ')') symbol += line1[i++];
    ...
    while(getline(ifs, line)){
        ...
        if(commandtype == 'A'){
            ...
            //L instruction
            string ss = line.substr(1, line.size() - 2); //还要去除末尾的换行符
